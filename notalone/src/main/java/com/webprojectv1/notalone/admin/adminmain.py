...
def get_sales_data():
    ...
    # 결과를 JSON 형식으로 변환
    sales_data = [{"month": row["month"], "total_sales": row["total_sales"]} for row in cursor.fetchall()]
    # 연결 종료
    cursor.close()
    conn.close()
...
def get_day5_data():
    ...
    cursor.execute(sql_query)
    # 결과를 JSON 형식으로 변환
    day5_data = [{"day": row["day"], "total_sales": row["total_sales"]} for row in cursor.fetchall()]
    # 연결 종료
    cursor.close()
    conn.close()
    return JSONResponse(content=day5_data)
